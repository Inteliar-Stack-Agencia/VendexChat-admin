-- Migration 043: Fix handle_new_user trigger for new user registration
--
-- Problem: Registration fails with "Database error saving new user".
-- Root cause: The trigger on auth.users fails when:
--   1. The requested slug is already taken (UNIQUE constraint violation)
--   2. Any other unhandled DB error blocks user creation entirely
-- Fix: Replace the trigger with a robust version that:
--   - Resolves slug conflicts by appending a numeric suffix
--   - Wraps execution in an EXCEPTION block so user creation never fails
--   - Falls back to a profileless insert if store creation still errors
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Store creation failed for an unexpected reason; still create the profile
    -- so the auth user is not left orphaned. The store can be created manually.
...
-- Re-create the trigger (drop first to replace safely)
