-- Migration 046: Fix update_my_store RPC authorization
-- The previous version only authorized via profile.store_id or email match.
-- This caused failures when profiles.store_id was not yet synced.
-- Added: owner_id check as a valid authorization path, and missing fields.
...
BEGIN
  ...
  -- Allow: superadmin, profile store match, email match, or owner_id match
