...
import { expensesApi, type Expense, type Supplier, type ExpenseCategory } from '../../services/expensesApi'
...
// ─── Setup Banner (if DB tables don't exist yet) ──────────────────────────────
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Controlá tus costos y gestioná tus proveedores</p>
            </div>
          </div>
          <Button
            ...
            {!dbError && !loading && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <Card className="p-4">
                  ...
                  <p className="text-2xl font-black text-indigo-600 mt-1">{formatPrice(totalPeriod)}</p>
                  <p className="text-[10px] text-gray-400 mt-1">{expenses.length} registros</p>
                </Card>
                {byCategory.slice(0, 3).map((c) => (
                  <Card key={c.value} className="p-4">
                    ...
                    <p className="text-xl font-black text-gray-800 mt-2">{formatPrice(c.total)}</p>
                    <p className="text-[10px] text-gray-400 mt-1">
                      {Math.round((c.total / (totalPeriod || 1)) * 100)}% del total
                    </p>
                  </Card>
                ))}
              </div>
                        ...
                        return (
                          <tr key={exp.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-3 font-medium text-gray-900">
                              {exp.description}
                              {exp.notes && (
                                <p className="text-xs text-gray-400 font-normal">{exp.notes}</p>
                              )}
                            </td>
                ...
                {filteredSuppliers.map((s) => (
                  <Card key={s.id} className="p-5 flex flex-col gap-3">
                    ...
                    <div className="space-y-1.5 text-xs text-gray-500">
                      ...
                      {s.notes && (
                        <p className="text-gray-400 italic">{s.notes}</p>
                      )}
                    </div>
                  </Card>
                ))}
