...
function CashForm({ date, session, onSave, onClose }: CashFormProps) {
  ...
  const loadFromPOS = async (silent = false) => {
    setLoadingPOS(true)
    try {
      ...
      if (!silent) showToast('success', 'Ventas del POS cargadas')
    } catch {
      if (!silent) showToast('error', 'Error al cargar ventas del POS')
    } finally {
      setLoadingPOS(false)
    }
  }
  ...
  useEffect(() => {
    // Al abrir un cierre nuevo, precargar solo los pedidos ya marcados como cobrados
    // ese día — así no depende de acordarse de apretar "Cargar desde POS".
    if (isNew && !autoLoadedRef.current) {
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  ...
                  {isNew ? 'Recargar pedidos cobrados' : 'Cargar desde Pedidos'}
                </button>
              </div>
...
export default function CashRegisterPage() {
  ...
  // Vengo del recordatorio del Dashboard ("Abrir caja") — abrir directo el cierre de hoy
  const handledOpenParam = useRef(false)
  useEffect(() => {
    if (loading || handledOpenParam.current || searchParams.get('open') !== 'today') return
    handledOpenParam.current = true
    if (todaySession) openEdit(todaySession)
    else openNew()
    setSearchParams((prev) => { prev.delete('open'); return prev }, { replace: true })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, todaySession, searchParams])
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Plus className="w-4 h-4" /> {todaySession ? 'Editar caja de hoy' : 'Abrir caja'}
            </Button>
          </div>
        </div>
