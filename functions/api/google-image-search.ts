...
async function runImageSearch(query: string, env: Env, request: Request) {
  const googleItems = await tryGoogleSearch(query, env)
  if (googleItems && googleItems.length > 0) {
    return json({ items: googleItems, source: 'google' }, 200, request)
  }

  const ddgItems = await tryDuckDuckGoSearch(query)
  if (ddgItems && ddgItems.length > 0) {
    return json({ items: ddgItems, source: 'duckduckgo' }, 200, request)
  }

  return json({ items: [], error: 'No se encontraron imágenes. Probá con otro término de búsqueda.' }, 200, request)
}
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  return runImageSearch(query, env, request)
}
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    if (!query?.trim()) {
      return json({ error: 'Query requerida.' }, 400, request)
    }
    return runImageSearch(query.trim(), env, request)
  } catch (error) {
